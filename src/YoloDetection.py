...
class YoloDetection:
    ...
    def __init__(self, n_cores=6, confThreshold=0.5, nmsThreshold=0.6, inpWidth=416, use_gpu=False):
        self.n_cores = n_cores

        # self.PATH_PERCEPTION_UTLITIES = rospkg.RosPack().get_path('perception_utilities')
        self.PATH_PERCEPTION_UTLITIES = "../"
        #For living room
        # self.CFG_PATH = self.PATH_PERCEPTION_UTLITIES+'/resources/yolo/yolov3-tiny_living_room.cfg'
        # self.WEIGHTS_PATH = self.PATH_PERCEPTION_UTLITIES+'/resources/yolo/yolov3-tiny_living_room.weights'
        # self.CLASS_NAMES_PATH = self.PATH_PERCEPTION_UTLITIES+'/resources/yolo/obj_living_room.names'

        # #For drinks
        # self.CFG_PATH = self.PATH_PERCEPTION_UTLITIES+'/resources/yolo/yolov3-tiny_drinks.cfg'
        # self.WEIGHTS_PATH = self.PATH_PERCEPTION_UTLITIES+'/resources/yolo/yolov3-tiny_drinks.weights'
        # self.CLASS_NAMES_PATH = self.PATH_PERCEPTION_UTLITIES+'/resources/yolo/obj_drinks.names'
    ...
    def Draw_detection(self, yolo_output, image):
        ...
        labels_identified = []
        x_coordinates_identified = []
        y_coordinates_identified = []
        for bbox in yolo_output[0]:
            color = (255, 0, 0)
            bbox = list(np.array(bbox))
            x, y, x2, y2, object_id, Name = bbox
            labels_identified.append(Name)
            x_coordinates_identified.append((int(x)+int(x2))/2)
            y_coordinates_identified.append((int(y)+int(y2))/2)
        ...
        return image_, labels_identified, x_coordinates_identified, y_coordinates_identified
