...
####################### CALLBACKS #######################
    ...
    def camera_compressed_subscriber(self, msg):
        ...
        self.Predictions = self.yoloDetect.Detection(imageData)
        result, labels, x_coordinates, y_coordinates = self.yoloDetect.Draw_detection(self.Predictions, imageData)
        # print(self.Predictions[0][0][:4])
        # print(self.depth)
        yolotroll = False
        try:
            x1, y1, x2, y2 = self.Predictions[0][0][:4]
            x1 = int(x1)
            y1 = int(y1)
            x2 = int(x1)
            y2 = int(y2)
        ...
        if not yolotroll:
            try:
                print(self.depth[y1][x1], self.depth[y2][x2])
                segmentation = depth.object_border(self.depth, [y1,x1,y2,x2])

                center = ((y1+y2)//2,(x1+x2)//2)
                result[center[0]][center[1]]=(255,0,0)
                for border_point in segmentation:
                    i, j = border_point
                    try:
                        result[i-10][j+10]=(0,255,0)
                    except:
                        # print("out of range")
                        # print(i+10,j-10)
                        pass
...
################################# PREPROCESSING SERVICES #################################
    ...
    def callback_save_image_srv(self, req):
        ...
        cv2.imwrite(self.PATH_IMAGES+'img/'+name+".jpg", cv2_img)
        with open(self.PATH_IMAGES+'depth/'+name+'.csv', mode = "w") as f:
...
##################################### OBJECT SERVICES #####################################
    ...
    def callback_object_recog_photo_srv(self,req):
        ...
        try:
            ...
            t2 = time.time()
            print(consoleFormatter.format("The photo has been processed successfully in "+str(t2-t1)+" seconds", "OKGREEN"))
            print(prediction)
