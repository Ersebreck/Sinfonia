...
#    hyp = yaml.load(f, Loader=yaml.loader.SafeLoader)
#mask_resolution = hyp['mask_resolution']
...
image = cv2.imread('../datos/uni.jpg')  # 504x378 image
...
pooler = ROIPooler(output_size=mask_resolution, scales=(pooler_scale,), sampling_ratio=1, pooler_type='ROIAlignV2', canonical_level=2)
...
base = bases[0]
bboxes = Boxes(pred[:, :4])
original_pred_masks = pred_masks.view(-1, mask_resolution, mask_resolution)
