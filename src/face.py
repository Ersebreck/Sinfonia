...
#             except:
#                 print('Error Leyendo la cara de:'+name)
#                 continue
...
def recognize_face(utilities,req):
    try:
        ...
        matches = face_recognition.compare_faces(faces_encodings_encodings,face_encodings,0.48)
        if True in matches:
...
def save_face(utilities,req,recog = False):
    t1 = time.time()
    timeout = req.num_pics+2
    print(consoleFormatter.format("Timeout of:"+str(timeout), "OKBLUE"))
    try:
        ...
        while stop_record!=True:
            ...
            if not os.path.exists(picsPersonPath):
                os.makedirs(picsPersonPath) 
            if(len(os.listdir(picsPersonPath)))>num_pics or time.time()-t1>timeout:
                if time.time()-t1>timeout:
                    print(consoleFormatter.format("Timeout  exceeded", "FAIL"))
                stop_record = True
